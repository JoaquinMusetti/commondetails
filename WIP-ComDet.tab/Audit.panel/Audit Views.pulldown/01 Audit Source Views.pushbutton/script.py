...
if total_missing == 0 and total_new == 0 and not master_missing:
    ...
    lines.append(u"Safe to proceed with Import Sheets with Views.")
    ui.show_report(
        text     = u"\n".join(lines),
        title    = u"Audit Source Views",
        subtitle = u"{} master views checked".format(len(data["master_views"])),
        summary  = u"✅ all in sync",
        context  = u"Compared the JSON's master_views section against the current "
                   u"model. The JSON's sheets section is not checked here — for "
                   u"that, use 'Audit Source Sheets'."
    )
    script.exit()
...
if not rename_suggestions:
    lines.append(u"")
    lines.append(u"Run Export Sheets with Views to refresh the JSON before importing.")
    ui.show_report(
        text     = u"\n".join(lines),
        title    = u"Audit Source Views",
        subtitle = u"{} master views checked".format(len(data["master_views"])),
        ...
        context  = u"No clean rename match was found — re-export the JSON or "
                   u"manually rename views to bring them in sync."
    )
    script.exit()
...
confirm = ui.confirm(
    ...
    yes_text="Confirm",
    context=u"renames.json is a side-car file that Import Sheets with Views "
            u"reads (via master_map.json convention) to remap view names "
            u"between source and destination."
)

if not confirm:
    lines.append(u"")
    lines.append(u"⏭  Renames not confirmed. Run Export Sheets with Views to refresh the JSON.")
    ui.show_report(
        text     = u"\n".join(lines),
        title    = u"Audit Source Views",
        subtitle = u"{} master views checked".format(len(data["master_views"])),
...
lines.append(u"Apply the renames in the destination model(s) and re-export the JSON.")
