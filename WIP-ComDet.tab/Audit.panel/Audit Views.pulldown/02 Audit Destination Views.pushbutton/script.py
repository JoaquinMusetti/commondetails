...
__doc__ = ('Compares a Sheets with Views JSON against the destination model. '
           ...
           'delete orphans and scale-mismatch views. Reads the combined JSON '
...
lines.append(u"Sheets in JSON:       {}".format(len(data.get("sheets", []))))
...
chosen_masters = ui.pick_list(
    ...
    context=u"Audits the active model (typically a building destination) against "
            ...
            u"that are missing from the model), scale/template mismatches, and "
...
for mv in data["master_views"]:
    if mv["view_name"] in chosen_masters_set:
        for dv in mv["dependent_views"]:
            ...
            json_view_placed[dv["view_name"]] = dv.get("placed_on_sheet", True)
...
if not has_scale_data:
    ...
    confirm_scale = ui.confirm(
        ...
        context=u"Scale mismatches usually mean the master view's scale was "
                u"changed in CD after this building's dependents were imported. "
                u"Re-importing recreates them with the correct scale."
    )
...
if not_in_json_orphan:
    confirm = ui.confirm(
        ...
        title="Audit Destination Views",
        context=u"True orphans (not in JSON AND not on a sheet) are almost "
                u"always safe to delete — they don't contribute to any sheet."
    )
...
if not_in_json_placed:
    confirm_placed = ui.confirm(
        ...
        title="Audit Destination Views",
        context=u"Placed views NOT in the JSON often indicate custom details that "
                u"should be migrated to CD first (via 'Export Sheets with Views' "
                u"in this building) before deleting them here."
    )
...
ui.show_report(
    ...
    context  = u"Run 'Audit Source Views' first if you also need to compare the "
               u"source-of-truth CD model against the JSON."
)
