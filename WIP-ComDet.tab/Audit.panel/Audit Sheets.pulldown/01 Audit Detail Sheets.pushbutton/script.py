...
__doc__ = ('Compares a Sheets with Views JSON against the destination model. '
           ...
           'offers to remove or delete orphaned viewports. Reads the combined '
           'JSON produced by "Export Sheets with Views".')
...
dest_prefix = ui.ask_for_string(
    ...
    context=u"Audits the sheet layout of the active model against the JSON. "
            ...
            u"to match sheet names — JSON sheets carry the 'CD' prefix from "
            u"the source export."
)
if not dest_prefix:
    script.exit()
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. Pre-audit: missing viewport types
# ─────────────────────────────────────────────────────────────────────────────
...
for sheet_data in layout:
    ...
    json_vp_dict = {entry["view_name"]: entry for entry in sheet_data["viewports"]}
...
# ─────────────────────────────────────────────────────────────────────────────
# 8. Handle orphaned viewports (interactive)
# ─────────────────────────────────────────────────────────────────────────────
...
if all_orphaned_vp_ids:
    action = ui.pick_list(
        ...
        button_name="Apply",
        multiselect=False,
        context=u"{} orphaned viewport(s) found on sheets — viewports not present "
                u"in the JSON. They will cause detail number conflicts during the "
                u"next import. 'Remove from sheets' unplaces the viewport but "
                u"keeps the view. 'Delete viewports' has the same effect (Revit "
                u"deletes the wrapper, the view remains as orphan). 'Do nothing' "
                u"leaves them for manual review.".format(len(all_orphaned_vp_ids))
    )
...
ui.show_report(
    ...
    summary  = u"✅ {}   ⚠ {}   ❌ {}".format(
        len(sheets_ok), len(sheets_with_issues), len(sheets_not_found)),
    context  = u"Pair with 'Audit Source Sheets' to compare two snapshots of the "
               u"JSON (Updates Report) when you've made changes upstream."
)
